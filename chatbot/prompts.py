...
def diagram(area, version, level, kind, subject, detail, context=""):
    return (
        ...
        "   - A labelled edge is written EXACTLY 'a -->|manages| b'. There is no "
        "second arrowhead after the closing pipe: '-->|manages|> b' is a parse "
        "error. Every edge must start with a source node id - never begin a line "
        "with '-->'.\n"
        "   - Declare each node once, then the edges, then any subgraphs. Keep "
        "the whole diagram to at most 20 nodes so it stays readable on a slide.\n"
